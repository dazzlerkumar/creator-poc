...
- [ ] **Verification Script**: Create a temporary script `scripts/verify-deps.ts` that attempts to import the key libraries.
- [ ] **Execution**: Run the script with `tsx` or `node` and confirm it fails because packages are missing.

### Phase 2: Green (Implementation)
- [ ] **Runtime Dependencies**: Install core packages:
    - `npm install zustand @tanstack/react-query centrifuge react-hook-form zod @hookform/resolvers react-virtuoso`
- [ ] **Dev Dependencies**: Install tooling and testing packages:
    - `npm install -D msw @tanstack/react-query-devtools`
- [ ] **Verify Existing**: Ensure `vitest`, `@testing-library/react`, and `@testing-library/jest-dom` are correctly configured in `package.json` (already present but verify versions).

### Phase 3: Refactor & Verify
- [ ] **Verification**: Run the `scripts/verify-deps.ts` script again to ensure all imports succeed.
- [ ] **Cleanup**: Remove the temporary verification script.
- [ ] **Lockfile**: Ensure `package-lock.json` is updated and committed.

---

## ✅ Acceptance Criteria (AC)
- [ ] **AC-001: Runtime Packages**: All packages listed in the implementation plan are present in `dependencies`.
- [ ] **AC-002: Dev Packages**: All tooling packages are present in `devDependencies`.
- [ ] **AC-003: Project Health**: `npm run dev` starts successfully without dependency errors.
...
- [ ] All requested packages are in `package.json`.
- [ ] `package-lock.json` is consistent.
- [ ] No console errors related to missing modules when starting the dev server.
