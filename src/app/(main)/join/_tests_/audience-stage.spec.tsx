...
vi.mock('@/app/(main)/join/_components/payment-overlay', () => ({
  PaymentOverlay: () => <div data-testid="mock-payment-overlay">Mock Payment</div>,
}));

vi.mock('@/app/(main)/join/_components/quiz-overlay', () => ({
  QuizOverlay: () => <div data-testid="mock-quiz-overlay">Mock Quiz</div>,
}));

vi.mock('@/app/(main)/join/_components/chat-header', () => ({
  ChatHeader: () => <div data-testid="mock-chat-header">Mock Chat Header</div>,
}));
...
describe('AudienceStage', () => {
  ...
  it('renders correctly and initializes youtube player immediately', () => {
    render(<AudienceStage videoBroadcastId="test-session" />);
